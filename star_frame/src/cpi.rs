...
pub trait HandleCpiArray {
    ...
    fn uninit_metas<'a>() -> Self::Arr<AccountRole<'a>>;
    /// # Safety
            ...
            paste::paste! {
                impl HandleCpiArray for typenum::[<U $n>] {
                    ...
                    unsafe fn invoke_signed<'a>(
                        ...
                        let metas: [AccountRole<'a>; $n] = metas.map(|meta|
                            // SAFETY:
...
impl HandleCpiArray for DynamicCpiAccountSetLen {
    ...
    unsafe fn invoke_signed<'a>(
        ...
        let metas_slice = unsafe {
            &*(core::ptr::from_ref::<[MaybeUninit<AccountRole<'a>>]>(metas_slice)
                as *const [AccountRole<'a>])
        };
        ...
        pinocchio::instruction::cpi::slice_invoke_signed(
            &InstructionView {
                program_id,
                data,
