...
pub trait SingleAccountSet {
    ...
    fn account_data_mut(&self) -> Result<RefMut<'_, [u8]>> {
        self.account_info().try_borrow_data_mut().with_ctx(|| {
            format!(
    ...
    T: SingleAccountSet + ?Sized,
{
    ...
    fn system_create_account(
        ...
        let account = *self.account_info();
        // SAFETY:
        // Allowed because we are checking the owner immediately and don't return the reference
        if !unsafe { account.owner() }.fast_eq(&System::ID) {
