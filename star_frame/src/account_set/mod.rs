...
/// An [`AccountSet`] that can be converted into a list of [`AccountView`]s and [`AccountRole`]s for a CPI.
///
/// # Safety
...
pub unsafe trait CpiAccountSet {
    ...
    fn write_account_metas<'a>(
        ...
        accounts: &'a Self::CpiAccounts,
        index: &mut usize,
        metas: &mut [MaybeUninit<AccountRole<'a>>],
    );
}
    ...
    T: CpiAccountSet,
{
    ...
    fn write_account_metas<'a>(
        ...
        _accounts: &'a Self::CpiAccounts,
        _index: &mut usize,
        _metas: &mut [MaybeUninit<AccountRole<'a>>],
    ) {
        unimplemented!()
    }
}
